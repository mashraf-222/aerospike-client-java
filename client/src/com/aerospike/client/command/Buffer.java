...
public final class Buffer {
	...
	*/
	...
	public static int estimateSizeUtf8Quick(String s) {
		...
		int len = s.length();
		int size = len; // Start with minimum (all ASCII)
		
		// Scan for non-ASCII characters to adjust estimate
		for (int i = 0; i < len; i++) {
			char c = s.charAt(i);
			if (c >= 0x80) {
				// Non-ASCII: add extra bytes
				if (c < 0x800) {
					size++; // 2-byte char (1 extra)
				} else if (Character.isHighSurrogate(c)) {
					size += 3; // 4-byte char (3 extra), skip next char
					i++;
				} else {
					size += 2; // 3-byte char (2 extra)
				}
			}
		}
	...
	public static Object bytesToBigInteger(byte[] buf, int offset, int len) {
		boolean negative = (buf[offset] & 0x80) != 0;

		if (negative) {
			// Create a copy with the sign bit cleared
			...
			// Positive number - create BigInteger directly from buffer slice
			byte[] bytes = new byte[len];
			System.arraycopy(buf, offset, bytes, 0, len);
	...
	public static long littleBytesToLong(byte[] buf, int offset) {
		// Process bytes in groups to reduce operations
		int low = (buf[offset] & 0xFF) |
		          ((buf[offset + 1] & 0xFF) << 8) |
		          ((buf[offset + 2] & 0xFF) << 16) |
		          ((buf[offset + 3] & 0xFF) << 24);
		
		int high = (buf[offset + 4] & 0xFF) |
		           ((buf[offset + 5] & 0xFF) << 8) |
